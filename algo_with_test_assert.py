...
def dfs1(v, graph, colors, vert):
    for nextv in graph[v]:
        if colors[nextv] == 0 and nextv in vert:
            colors[nextv] = 1
            dfs1(nextv, graph, colors, vert)


def unreach1(graph, vert, n):
    colors = [1 for _ in range(n)]
    for v in vert:
        colors[v] = 0
    first = vert[0]
    colors[first] = 1
    dfs1(first, graph, colors, vert)
    for c in colors:
        if c == 0:
            return False
    return True
...
for v in tqdm(range(n)):
    ...
    if ansi[0] < ans:
        ans = ansi[0]
        graphs = ansi[1]
assert check_dom(graph1, [i for i in graphs], n) and check_conn(graph1, [i for i in graphs], n)
...
for i in range(2 ** n):
    ...
    if check_dom(graph1, vert, n) and check_conn(graph1, vert, n):
        if len(vert) < ans:
            ans = len(vert)
            graphs = vert
graphs = {i for i in graphs}
print('Полный перебор:')
print('Размер множества: ', ans)
print('Искомое множество вершин из полного перебора:', graphs)